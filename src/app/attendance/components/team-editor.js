...
export default function TeamEditor({ team, onClose, onSaved, onDeleteTeam }) {
  ...
  const initialTeam = team || {
    name: "",
    room: null,
    members: [{ ...emptyMember(), role: "leader" }],
  };
  ...
  const [draft, setDraft] = useState({
    ...initialTeam,
    members: initialTeam.members.map((member) => ({ ...member })),
  });
  ...
  const makeLeader = (index) =>
    setDraft((current) => ({
      ...current,
      members: current.members.map((member, memberIndex) => ({
        ...member,
        role: memberIndex === index ? "leader" : "member",
      })),
    }));
  ...
  async function save() {
    ...
    try {
      const response = await fetch(
        isNew ? "/api/teams" : "/api/teams/" + team.id,
        {
    ...
    <>
            ...
            <Modal.Dialog className="editor-modal">
              <Modal.Header className="modal-heading editor-modal-heading">
                <div>
                  ...
                  <Modal.Heading className="editor-title">
                    {isNew ? "Create team" : "Edit #" + team.id}
                  </Modal.Heading>
                </div>
                <Modal.CloseTrigger
              ...
              <Modal.Body className="editor-modal-body">
                ...
                {!isNew && (
                  <label className="field-label room-field">
                    ROOM
                    <DropdownField
                      value={draft.room}
                ...
                <p className="editor-note">
                  Choose one leader. They are always stored first in the roster.
                </p>
                    ...
                    >
                      <div className="editor-member-bar">
                        ...
                        <div className="editor-member-actions">
                          <Switch
                            size="sm"
                            className="leader-switch"
                            isSelected={member.role === "leader"}
                            onChange={(isSelected) => {
                              if (isSelected) makeLeader(index);
                            }}
                          >
                            <Switch.Content>
                              <Switch.Control>
                          ...
                          <label className="field-label" key={key}>
                            ...
                            {key === "branch" ? (
                              <DropdownField
                                ...
                                options={[
                                  ...(member.branch &&
                                  !branchOptions.includes(member.branch)
                                    ? [
                                        {
                                          value: member.branch,
                                          label: member.branch,
                              ...
                              <DropdownField
                                value={member.year ? String(member.year) : ""}
                                onChange={(value) =>
                                  updateMember(index, key, value)
                                }
              ...
              <Modal.Footer className="modal-actions">
                {!isNew && (
                  <Button
                    variant="ghost"
                    className="remove-member"
                    onPress={() => setConfirmation({ type: "team" })}
                  >
      ...
      <Modal state={confirmState}>
        <Modal.Backdrop variant="blur">
          <Modal.Container>
            <Modal.Dialog className="confirmation-modal">
              <Modal.Header>
                <Modal.Heading>
                  {confirmation?.type === "team"
                    ? "Delete team?"
                    : "Remove participant?"}
                </Modal.Heading>
              </Modal.Header>
              <Modal.Body>
                <p>
                  {confirmation?.type === "team"
                    ? "This permanently deletes the team and every participant in it."
                    : `Remove ${confirmation?.name} from this team? Their participant record will be permanently deleted when changes are saved.`}
                </p>
              </Modal.Body>
              <Modal.Footer>
                <Button variant="ghost" onPress={() => setConfirmation(null)}>
                  Cancel
                </Button>
                <Button variant="danger" onPress={confirmDeletion}>
                  Delete
                </Button>
              </Modal.Footer>
            </Modal.Dialog>
